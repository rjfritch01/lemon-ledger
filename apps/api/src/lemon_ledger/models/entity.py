...
_BRIDGE_TREATMENTS = "bridge_treatment IN ('relocate','disposition')"
...
class Entity(UUIDPrimaryKeyMixin, Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(_BRIDGE_TREATMENTS, name="ck_entities_bridge_treatment"),
    )
    ...
    # 1.8 seams
    jurisdiction: Mapped[str] = mapped_column(Text, server_default="US", nullable=False)
    bridge_treatment: Mapped[str] = mapped_column(Text, server_default="relocate", nullable=False)
